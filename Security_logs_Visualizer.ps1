<#
    ...
    - UPDATED: "Clear" button renamed to "Clear Logs"
    - UPDATED: Enabled Cell Selection for Copy/Paste (Ctrl+C)
    - UPDATED: Isolated SHA256 from Sysmon hash strings
    - FIXED: Performance optimized HTML Engine & Date Filtering
#>
...
function Get-CombinedEDREvents {
    ...
    $parsedData = foreach ($e in $events) {
        ...
        # SHA256 Extraction
        $sha256 = "N/A"
        if ($data.Hashes -match 'SHA256=([A-Fa-f0-9]{64})') {
            $sha256 = $Matches[1]
        } elseif ($data.Hashes -match '^[A-Fa-f0-9]{64}$') {
            $sha256 = $data.Hashes
        }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="10">
        ...
        <Border Grid.Row="1" Background="#E9ECEF" Padding="10" CornerRadius="5" Margin="0,0,0,10">
            <WrapPanel>
                ...
                <StackPanel Margin="0,0,10,0">
                    ...
                    <TextBox x:Name="TbTreeFilt" Width="200" Height="25"/>
                </StackPanel>
        ...
        <StatusBar Grid.Row="3" Background="#F0F0F0">
            <StatusBarItem><TextBlock x:Name="TxtStatus" Text="Ready - Ctrl+C to copy selected cells"/></StatusBarItem>
        </StatusBar>
    </Grid>
</Window>
"@
